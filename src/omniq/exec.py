...
class Exec:
    ...
    def pause(self, *, queue: str) -> str:
        return self.ops.pause(queue=queue)

    def resume(self, *, queue: str) -> int:
        return self.ops.resume(queue=queue)

    def is_paused(self, *, queue: str) -> bool:
        return self.ops.is_paused(queue=queue)
    
    def childs_init(self, key: str, expected: int) -> None:
        self.ops.childs_init(key=key, expected=int(expected))
    ...
    def child_ack(self, key: str, child_id: Optional[str] = None) -> int:
        ...
        return int(self.ops.child_ack(key=key, child_id=cid))
